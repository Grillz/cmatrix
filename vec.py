...
def add(u,v):
    ...
    """
    assert u.D == v.D
    return Vec(v.D, {i: (u.f.get(i,0) + v.f.get(i,0)) for i in v.D})
...
def dot(u,v):
    ...
    """
    assert u.D == v.D
    return sum([v.f.get(i, 0) * u.f.get(i, 0) for i in v.D])
...
def scalar_mul(v, alpha):
    ...
    True
    """
    return Vec(v.D, {i: (v.f.get(i, 0) * alpha) for i in v.D})
...
def neg(v):
    ...
    True
    """
    return Vec(v.D, {i:(-1 * v.f.get(i, 0)) for i in v.D})
